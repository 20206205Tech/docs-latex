...
% \item % \textbf{Nâng cao chất lượng AI và tối ưu hóa RAG:}
% Nghiên cứu huấn luyện tinh chỉnh (Fine-tuning)
% mô hình trên tập dữ liệu luật Việt Nam;

% \item % Sử dụng Airflow cho luồng dữ liệu
% Hỗ trợ Directed Acyclic Graph
% định nghĩa các luồng logic
% dựa trên điều kiện của dữ liệu thực tế.

% \item Sử dụng ragas để test về ai => cấu hình ngưỡng hợp lý.

% \item Sử dụng test-playwright để test về api, giao diện giúp phát hiện lỗi đảm bảo hệ thống hoạt động hiệu năng tốt

% \item % Dịch vụ thanh toán
% thêm chức năng xuất hóa đơn cho người trong công ty
% thêm chức năng giảm giá xử lý bài toán Nhiều người mua cùng lúc
% thêm cổng thanh toán Stripe cho người dùng nước ngoài có nhu cầu

% \item % Thuê thêm các dịch vụ chuyển đổi giọng nói
% hoặc chạy triển khai mô hình
% Sử dụng GPU
% Giảm thiểu tối đa độ trễ nâng cao trải nghiệm

% \item % Tích hợp công nghệ xử lý văn bản đa dạng các định dạng

% \item % % % kubernetes nhiều cụm, môi trường đám mây
% tối ưu hóa quy trình tự động co giãn (Auto-scaling) trên cụm Kubernetes
% nhằm đảm bảo tính sẵn sàng cao khi có lượng lớn người dùng truy cập đồng thời.
