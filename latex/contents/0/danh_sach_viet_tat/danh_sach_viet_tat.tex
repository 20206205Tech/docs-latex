\newpage
{\centering \section*{Danh sách viết tắt}} % Đặt tên tiêu đề
\addcontentsline{toc}{section}{Danh sách viết tắt} % Thêm vào mục lục

\begin{table}[ht]
\centering
\begin{tabular}{|c|c|c|l|}
\hline
\textbf{STT} & \textbf{Từ viết tắt} & \textbf{Từ viết đầy đủ} & \textbf{Mô tả} \\ \hline
1 & AI & Artificial Intelligence & Trí tuệ nhân tạo \\ \hline
2 & LLM & Large Language Model & Mô hình ngôn ngữ lớn \\ \hline
3 & NLP & Natural Language Processing & Xử lý ngôn ngữ tự nhiên \\ \hline
4 & RAG & Retrieval Augmented Generation & Tạo tăng cường truy xuất \\ \hline
\end{tabular}
\end{table}

\newpage
