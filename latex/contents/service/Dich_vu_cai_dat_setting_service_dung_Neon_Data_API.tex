...
\begin{figure}[H]
\centering
\includegraphics[width=0.99\textwidth]{pictures/setting-neonimage-2.png}
...
key & text & Khóa/Từ khóa định nghĩa thuộc tính cài đặt. \\ \hline
value & text & Giá trị của cài đặt tương ứng. \\ \hline
updated\_at & timestamptz & Thời gian cập nhật cấu hình lần cuối. \\ \hline
\end{tabular}
\caption{Cấu trúc bảng \texttt{settings} (Dịch vụ cài đặt)}
\end{table}
\paragraph{Cấu hình trong Kong API Gateway}

\begin{itemize}

\item Tạo Service trong Kong trỏ trực tiếp đến endpoint REST API của bảng \texttt{settings} trên Neon.
\item Thay vì tạo một Route chung cho tất cả các yêu cầu,
hệ thống được cấu hình để phân loại riêng biệt luồng đọc (\texttt{GET}) và luồng ghi (\texttt{POST}).
\item Tham số \texttt{strip\_path=true} giúp Kong loại bỏ tiền tố URI trước khi chuyển tiếp yêu cầu đến Neon.
\item Cấu hình Plugin xử lý Upsert tự động:
...
\item Sử dụng plugin \texttt{request-transformer}
để Kong API Gateway
