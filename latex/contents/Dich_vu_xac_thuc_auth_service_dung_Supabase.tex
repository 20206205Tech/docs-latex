...
\begin{itemize}
\item Khởi tạo quá trình liên kết thiết bị xác thực mới. API trả về mã QR Code (\texttt{qr\_code}), mã bí mật (\texttt{secret}) và chuỗi URI để người dùng quét trên ứng dụng Authenticator.
\item Endpoint sử dụng: \texttt{/factors}
\end{itemize}
...
\item Endpoint sử dụng: \texttt{/factors/\{factorId\}/challenge}
\end{itemize}
...
\item Endpoint sử dụng: \texttt{/factors/\{factorId\}/verify}
\end{itemize}
...
\textbf{Thuật toán TOTP (Time-based One-time Password)} sử dụng yếu tố thời gian Unix (thay vì dùng bộ đếm) (thời gian $T_0$ của Unix là ngày 1 tháng 1 năm 1970). Giá trị của bộ đếm trong thuật toán TOTP được tính bằng công thức:
...
Tóm lại, TOTP sử dụng khóa bí mật có thể hiển thị dưới dạng mã QR cho người dùng. Từ đó, máy chủ và phần mềm Authenticator đều có thể tính được giá trị của mã TOTP theo thuật toán. Cuối cùng, khi cần xác thực thì người dùng nhập giá trị TOTP hiển thị trên phần mềm Authenticator để máy chủ tiến hành so sánh.
