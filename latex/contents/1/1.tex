...
\begin{figure}[H]
\centering
\includegraphics[scale = 0.2]{pictures/rag-1.excalidraw.png}
...
\item Toàn bộ dữ liệu tri thức (tài liệu, hướng dẫn, văn bản luật...) được chia nhỏ thành các đoạn (chunks).
...
\begin{figure}[H]
\centering
\includegraphics[scale = 0.2]{pictures/rag-2.excalidraw.png}
...
\item Nodes (Nút): Đại diện cho các bước, tác vụ (tasks) hoặc hành động cụ thể (ví dụ: gọi LLM, tìm kiếm cơ sở dữ liệu, chào người dùng).
\item Edges (Cạnh): Định nghĩa luồng di chuyển giữa các node. Edges có thể là đường đi trực tiếp hoặc có điều kiện (conditional edges - dựa trên kết quả của node trước để quyết định đi tiếp đến node nào).
\item Cycles (Vòng lặp): Đây là tính năng quan trọng nhất. Khác với kiến trúc DAG (Đồ thị có hướng không chu trình) thông thường, LangGraph cho phép agent quay lại các node trước đó để thực hiện lại, tự sửa lỗi, hoặc lặp lại quy trình (iterative process) cho đến khi đạt mục tiêu.
\end{itemize}
...
\begin{figure}[H]
\centering
\includegraphics[scale = 0.25]{pictures/rag_diagram.png}
