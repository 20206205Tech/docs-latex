...
Sự kiện này được tiêu thụ (consume) bởi hai dịch vụ:
\textbf{Dịch vụ trò chuyện (conversation service)}
và \textbf{Dịch vụ văn bản (document service)}
để đồng bộ quyền lợi gói VIP của người dùng.

\textbf{Dịch vụ văn bản (document service - Python)}
...
\item Nếu phiên bản nhận được $>$ phiên bản hiện tại: Tiến hành cập nhật thông tin thuê bao (\texttt{period\_start}, \texttt{period\_end}, \texttt{subscription\_id}, \texttt{version}) và đặt trạng thái \texttt{is\_active = True}.
\item Nếu phiên bản nhận được $\le$ phiên bản hiện tại: Bỏ qua (SKIP) sự kiện vì đã tồn tại bản cập nhật mới hơn trong DB (ngăn chặn lỗi out-of-order event).
\end{itemize}
\item Người dùng chính thức được nâng quyền sử dụng dịch vụ bóc tách và tóm tắt văn bản dành riêng cho VIP.
\end{enumerate}
\end{itemize}

\textbf{Dịch vụ trò chuyện (conversation service - NestJS)}
...
IPN (Instant Payment Notification) được sử dụng để
