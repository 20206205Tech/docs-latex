...
Người dùng chọn kích hoạt xác thực hai lớp. Giao diện gửi yêu cầu đăng ký qua cổng API đến dịch vụ chứng thực. Dịch vụ chứng thực đăng ký thiết bị xác thực mới ở trạng thái chưa kiểm chứng, đồng thời tạo mã khóa bảo mật và hình ảnh mã phản hồi nhanh gửi về giao diện.
\item \textbf{Đồng bộ hóa ứng dụng:}
Người dùng quét hình ảnh mã phản hồi nhanh bằng ứng dụng xác thực trên điện thoại để thiết lập đồng bộ thời gian. Ứng dụng này sẽ liên tục tạo ra các mã xác thực gồm sáu chữ số thay đổi mỗi ba mươi giây.
\item \textbf{Nhập mã kiểm chứng:}
Người dùng nhập mã xác thực hiển thị trên ứng dụng để kiểm tra. Giao diện gửi yêu cầu tạo mã thách thức và gửi mã xác thực kèm mã thách thức qua cổng API tới dịch vụ chứng thực để xác minh.
\item \textbf{Kích hoạt thành công:}
Nếu mã xác thực chính xác, dịch vụ chứng thực chuyển trạng thái thiết bị xác thực thành đã xác minh, đồng thời cấp lại mã đăng nhập mới với mức độ bảo mật cao hơn cho người dùng.
\end{itemize}
