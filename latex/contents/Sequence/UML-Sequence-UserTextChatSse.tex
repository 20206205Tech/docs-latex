...
\item \textbf{Kiểm tra VIP:} Khi \texttt{use\_reasoning=true}, Conversation Service gọi \texttt{isVip(userId)} trên bảng \texttt{user\_subscriptions}. Subscription hợp lệ khi \texttt{is\_active=true} và \texttt{period\_end >= now}; nếu không hợp lệ, service trả \texttt{ForbiddenException}.
...
\item \textbf{Gọi Chatbot Service:} Conversation Service gọi gRPC và truyền token qua metadata.
\item \textbf{Streaming kết quả:} Chatbot Service verify token, lưu human message, rồi stream các gói thông tin trả lời.
