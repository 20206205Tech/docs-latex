\paragraph{Sơ đồ tuần tự Pha Outbox Relay publish sự kiện mua VIP lên Kafka}

Pha này mô tả \texttt{OutboxRelayCron} của Payment Service đọc các bản ghi Outbox chưa xử lý, kiểm tra payload của \texttt{SubscriptionPurchasedEvent}, publish sự kiện mua gói VIP lên Kafka thông qua \texttt{MessageBrokerPort}, rồi cập nhật trạng thái Outbox.

\begin{figure}[H]
\centering
\includegraphics[scale = 0.18]{contents/Sequence/UML-Sequence-32.png}
\caption{Sơ đồ tuần tự Pha Outbox Relay publish sự kiện mua VIP lên Kafka}
\end{figure}
...
\end{tabular}
\caption{Các thành phần tham gia pha Outbox Relay publish sự kiện mua VIP lên Kafka}
\end{table}
